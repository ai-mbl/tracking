...
assert len(matches) == 2, "You should only produce two matches, since the maximal cost of links is 7."
print("Success :)")
...
class BipartiteMatchingLinker(FrameByFrameLinker):    
    ...
    def _link_two_frames(self, cost_matrix):
        ...
        ### YOUR CODE HERE ###
        ######################
        deaths = np.full(shape=(cost_matrix.shape[0], cost_matrix.shape[0]), fill_value=0)
        ...
        ### YOUR CODE HERE ###
        ######################
        births = np.full(shape=(cost_matrix.shape[1], cost_matrix.shape[1]), fill_value=0)
